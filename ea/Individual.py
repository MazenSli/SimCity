...
class Individual:
    ...
    def __init__(self):
        ...
        self.mutRate = [self.uniprng.uniform(0.9, 0.1) for k in range(2)]  # use "normalized" sigma
...
class MultivariateIndividual(Individual):
    ...
    def __init__(self):
        ...
        for i in range(self.nLength):
            ...
            self.state[1].append(self.uniprng.uniform(0, self.state[0][i] * 40))
    ...
    def crossover(self, other):
        ...
        for i in range(self.nLength):
            ...
            # toggle shift limits
            if self.state[1][i] > self.state[0][i] * 40:
                self.state[1][i] = self.state[0][i] * 40
            if self.state[1][i] < 0:
                self.state[1][i] = 0
            if other.state[1][i] > self.state[0][i] * 40:
                other.state[1][i] = self.state[0][i] * 40
            if other.state[1][i] < 0:
                other.state[1][i] = 0
    ...
    def mutate(self):
        ...
        for i in range(self.nLength):
            ...
            self.state[1][i] = self.state[1][i] + \
                               (self.state[0][i] * 40 - 0) * self.mutRate[1] \
    ...
    def __str__(self):
        str_ind = ''
        params = ['NorthGreenRatio', 'ToggleTime']
